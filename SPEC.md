...
| B9 | x | `guardrails / check` fails because the standalone `confirm` app runs without the devShell hook that materializes the ignored `lefthook.yml`, causing completeness and executability checks to fail. Fix: have the `confirm` app copy its generated `lefthook.yml` into the project before invoking the confirmation script. |

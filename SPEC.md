...
| B9 | x | `guardrails / check` fails because the `actions` fragment was detected but omitted from the declared materialization fragments, leaving generated `lefthook.yml` out of sync. Fix: declare `actions` in both fragment lists and regenerate `lefthook.yml`. |
